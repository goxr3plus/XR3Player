			...
			protected Boolean call() throws Exception {
				...
				try {
					...
					copyThread = new Thread(() -> {
						// Start File Copy
						try {
							zip = FileChannel.open(pathToLocalResource.get(), StandardOpenOption.CREATE, StandardOpenOption.TRUNCATE_EXISTING,
					...
					while ( ( outPutFileLength = destinationFile.length() ) < totalBytes && !stopThread) {
						...
						System.out.println("Current Bytes:" + outPutFileLength + " ,|, TotalBytes:" + totalBytes + " ,|, Current Progress: "
								+ ( outPutFileLength * 100 ) / totalBytes + " %");
