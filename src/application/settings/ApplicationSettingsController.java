...
public class ApplicationSettingsController extends BorderPane {
	...
	private void initialize() {
		...
		restoreDefaults.setOnAction(a -> {
			...
			if (ActionTool.doQuestion("Soore you want to restore defaults for the selected category of settings", restoreDefaults, Main.window)) {
		...
		restoreAllSettings.setOnAction(a -> {
			...
			if (ActionTool.doQuestion("Soore you want to restore <All The Settings> to default", restoreAllSettings, Main.window)) {
				...
				ActionTool.deleteFile(new File(InfoTool.getAbsoluteDatabasePathWithSeparator() + Main.userMode.getUser().getUserName() + File.separator + "settings"
						+ File.separator + InfoTool.USER_SETTINGS_FILE_NAME));
				
				//Lock the update properties
				Main.dbManager.getPropertiesDb().setUpdatePropertiesLocked(false);
				
				ActionTool.showNotification("Settings Restored", "All the settings are restored to default", Duration.seconds(2), NotificationType.INFORMATION);
			}
		});
	}
