	...
	protected void maximize() {
		...
		try {
			if (Screen.getScreensForRectangle(stage.getX(), stage.getY(), stage.getWidth() / 2, stage.getHeight() / 2).isEmpty())
				...
				screen = Screen.getScreensForRectangle(stage.getX(), stage.getY(), stage.getWidth() / 2, stage.getHeight() / 2).get(0)
						.getVisualBounds();
	...
	private void setResizeControl(Pane pane , final String direction) {
		...
		pane.setOnMouseClicked(m -> {
			if ( ( m.getButton().equals(MouseButton.PRIMARY) ) && ( m.getClickCount() == 2 )
				...
				Rectangle2D screen = Screen.getScreensForRectangle(stage.getX(), stage.getY(), stage.getWidth() / 2, stage.getHeight() / 2).get(0)
						.getVisualBounds();
