...
public class XPlayerController extends StackPane implements DJDiscListener, StreamPlayerListener {
			...
			if (key == 1 || key == 2) {
				if (djMode.balancer.getVolume() < 100) { // <100
					...
					Main.xPlayersList.getXPlayer(1)
					        .setGain( ( ( Main.xPlayersList.getXPlayerUI(1).getVolume() / 100.00 )
					                * ( djMode.balancer.getVolume() ) ) / 100.00);
					...
					Main.xPlayersList.getXPlayer(2)
					        .setGain( ( ( Main.xPlayersList.getXPlayerUI(2).getVolume() / 100.00 )
			...
			return new Task<Boolean>() {
				...
				protected Boolean call() throws Exception {
					...
					try {
						...
						// System.out.println("Continuing with PlayService after
						// Stop Call!")
	...
	@Override
	public void progress(int nEncodedBytes , long microSecondsPosition , byte[] pcmdata ,
