...
public class LibraryMode extends GridPane {
    ...
    private final InvalidationListener creationInvalidator = new InvalidationListener() {
        ...
        public void invalidated(Observable observable) {
            ...
            if (!Main.renameWindow.isShowing() && !Main.renameWindow.isXPressed()) {
                ...
                // if can pass
                if (!libraryViewer.items.stream()
                    .anyMatch(lib -> lib.getLibraryName()
                        .equals(name))) {
                    ...
                    try {
                        ...
                        Main.dbManager.connection1.createStatement()
                            .executeUpdate("CREATE TABLE '" + dataBaseTableName + "'" + "(PATH       TEXT    PRIMARY KEY   NOT NULL ," + "STARS       DOUBLE     NOT NULL," + "TIMESPLAYED  INT     NOT NULL," + "DATE        TEXT   	NOT NULL,"
                                + "HOUR        TEXT    NOT NULL)");
                        ...
                        insertNewLibrary.setDouble(3, currentLib.starsProperty()
                            .get());
                    ...
                    Notifications.create()
                        ...
                        .text("A Library or PlayList with this name already exists!")
                        .darkStyle()
                        .showConfirm();
                }
            }
    ...
    public LibraryMode() {
        ...
        // Prepared Statement
        try {
            insertNewLibrary = Main.dbManager.connection1.prepareStatement("INSERT INTO LIBRARIES (NAME,TABLENAME,STARS,DATECREATED,TIMECREATED,DESCRIPTION,SAVEMODE,POSITION,LIBRARYIMAGE,OPENED) " + "VALUES (?,?,?,?,?,?,?,?,?,?)");
    ...
    public void initialize() {
        ...
        createLibrary.setOnAction(a -> {
            if (!Main.renameWindow.isShowing()) {
                ...
                Main.renameWindow.showingProperty()
                    .addListener(creationInvalidator);
            }
        });
        ...
        newLibrary.visibleProperty()
            .bind(Bindings.size(libraryViewer.items)
                .isEqualTo(0));
        ...
        // StackPane
        librariesStackView.getChildren()
            .addAll(libraryViewer, librariesSearcher.region, librariesSearcher.searchProgress);
        ...
        Main.xPlayersList.getXPlayerUI(0)
            .makeTheDisc(136, 136, Color.ORANGE, 45, Side.LEFT);
        Main.xPlayersList.getXPlayerUI(0)
            .makeTheVisualizer(Side.RIGHT);
    ...
    public class LibrariesViewer extends Region {
            ...
            this.setOnMouseMoved(m -> {
                ...
                if (dragDetected) {
                    ...
                    try {
                        ...
                        robot.mouseMove((int) m.getScreenX(), (int) this.localToScreen(this.getBoundsInLocal())
                            .getMinY() + 2);
                ...
                if (m.getButton() == MouseButton.PRIMARY || m.getButton() == MouseButton.MIDDLE) {
                    ...
                    if (((Library) centerGroup.getChildren()
                        ...
                        timeline.setOnFinished(v -> {
                            ...
                            contextMenu.show(Main.window, bounds.getMinX() + bounds.getWidth() / 3, bounds.getMinY() + bounds.getHeight() / 4, library);
        ...
        public void calculateCenterAfterDelete() {
            ...
            // center index
            if (!leftGroup.getChildren()
                .isEmpty())
                centerIndex = leftGroup.getChildren()
                    .size() - 1;
            else
        ...
        public void update() {
            ...
            // Reconstruct Groups
            leftGroup.getChildren()
                .clear();
            centerGroup.getChildren()
                .clear();
            rightGroup.getChildren()
                .clear();
            ...
            if (!items.isEmpty()) {
                ...
                if (items.size() == 1) {
                    ...
                    if (centerIndex == items.size()) {
                        centerGroup.getChildren()
                            .add(leftGroup.getChildren()
                                .get(centerIndex - 1));
                    } else
                        centerGroup.getChildren()
                            .add(items.get(centerIndex));
                ...
                // LEFT KEYFRAMES
                for (int i = 0; i < leftGroup.getChildren()
                    .size(); i++) {

                    final Library it = items.get(i);

                    double newX = -leftGroup.getChildren()
                        .size() *
                ...
                if (items.size() == 1)
                    ...
                    centerItem = (Library) centerGroup.getChildren()
                        .get(0);
                ...
                // RIGHT KEYFRAMES
                for (int i = 0; i < rightGroup.getChildren()
                    .size(); i++) {

                    final Library it = items.get(items.size() - i - 1);

                    final double newX = rightGroup.getChildren()
                        .size() *
