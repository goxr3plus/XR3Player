...
public class LibrariesSearcher extends HBox {
	...
	public LibrariesSearcher() {
		...
		searchField.textProperty().addListener((observable , oldValue , newValue) -> service.startService());
	...
	public void registerListeners() {
		...
		Main.window.xProperty().addListener((observable , oldValue , newValue) -> recalculateWindowPosition());
		Main.window.yProperty().addListener((observable , oldValue , newValue) -> recalculateWindowPosition());
		Main.window.widthProperty().addListener((observable , oldValue , newValue) -> recalculateWindowPosition());
		Main.window.heightProperty().addListener((observable , oldValue , newValue) -> recalculateWindowPosition());
	}
		...
		protected Task<Void> createTask() {
			return new Task<Void>() {
				...
				protected Void call() throws Exception {
					...
					// matcher
					Main.libraryMode.libraryViewer.items.stream().forEach(lib -> {
						// match?
						if (lib.getLibraryName().toLowerCase().contains(word)) {
							Platform.runLater(() -> libSearchWinController
							        .addChildren(new ResultButton(lib.getLibraryName(), lib.getPosition())));
							++found;
						}
					});
					return null;
