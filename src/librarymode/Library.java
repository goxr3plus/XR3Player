...
public class Library extends StackPane {
				...
				try {
					...
					if (!Main.renameWindow.isXPressed() && !libraryName.equals(newName)) {
						...
						if (! ( duplicate = Main.libraryMode.libraryViewer.items.stream().anyMatch(
							...
							if (imagePath != null)
								updateImage(Main.dbManager.imagesFolder + File.separator + newName + "."
	 ...
	 */
	private void updateImage(String newPath , boolean renameUpdate , boolean commit) {
		try {
	...
	public void setDefaultImage() {
		...
		try {
			...
			// Set the default
			imageView.setImage(Main.libraryMode.defaultImage);
	...
	public void changeImage() {
		...
		if (file != null) {
			...
			if (image.getWidth() <= 4800 && image.getHeight() <= 4800) {
				...
				updateImage(file.getAbsolutePath(), false, true);
			} else
