 ...
 * @author GOXR3PLUS
 */
public class Searcher extends HBox {
	 ...
	 */
	public Searcher(SmartController control) {
		controller = control;
		...
		protected Task<Void> createTask() {
			return new Task<Void>() {
				...
				protected Void call() throws Exception {
					...
					pageBeforeSearch = controller.listNumber.get();
					controller.listNumber.set(0);
					...
					try (ResultSet set = Main.dbManager.connection1.createStatement().executeQuery("SELECT* FROM '"
						...
						while (set.next()) {
							song = new Audio(set.getString("PATH"),
							        InfoTool.durationInSeconds(set.getString("PATH"), TYPE.FILE),
