...
public class VisualizerWindow extends StackPane {
    ...
    public VisualizerWindow(XPlayerController xPlayerUI) {
        ...
        try {
            ...
            Logger.getLogger(getClass().getName())
                .log(Level.SEVERE, "VisualizerWindow FXML can't be loaded!", ex);
        }
    ...
    private void initialize() {
        ...
        window.widthProperty()
            .addListener((observable, oldValue, newValue) -> {
                if (newValue.intValue() <= 300 && progressBarStackPane.isVisible()) {
                    progressBarStackPane.setVisible(false);
                    progressBarStackPane.setManaged(false);
                    moreMenuButton.setVisible(false);
                    moreMenuButton.setManaged(false);
                    onTop.setVisible(false);
                    onTop.setManaged(false);
                    transparencySlider.setVisible(false);
                    transparencySlider.setManaged(false);
                } else if (newValue.intValue() > 300 && !progressBarStackPane.isVisible()) {
                    progressBarStackPane.setVisible(true);
                    progressBarStackPane.setManaged(true);
                    moreMenuButton.setVisible(true);
                    moreMenuButton.setManaged(true);
                    onTop.setVisible(true);
                    onTop.setManaged(true);
                    transparencySlider.setVisible(true);
                    transparencySlider.setManaged(true);
                }
            });
    ...
    public void changeImage(Type type) {
        ...
        if (image != null) {
            Image img = new Image(image.toURI()
                .toString());
            if (img.getWidth() <= 4800 && img.getHeight() <= 4800) {
                ...
                ActionTool.showNotification("Warning", "Maximum Size Allowed 4800*4800 \n Current is:" + img.getWidth() + "*" + img.getHeight(), Duration.millis(1500), NotificationType.WARNING);
