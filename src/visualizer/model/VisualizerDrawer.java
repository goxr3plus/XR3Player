...
public class VisualizerDrawer extends VisualizerModel {
    ...
    /** The yellow light. */
    private final static Image yellowLight = new Image(VisualizerModel.class.getResourceAsStream("yellowLight.png"));

    /** The blue light. */
    private final static Image blueLight = new Image(VisualizerModel.class.getResourceAsStream("blueLight.png"));

    /** The grey light. */
    private final static Image greyLight = new Image(VisualizerModel.class.getResourceAsStream("greyLight.png"));

    /** The light blue light. */
    private final static Image lightBlueLight = new Image(
	    VisualizerModel.class.getResourceAsStream("lightBlueLight.png"));

    /** The red light. */
    private final static Image redLight = new Image(VisualizerModel.class.getResourceAsStream("redLight.png"));
	...
	if (stereo)
	    ...
	    pSample1 = stereoMerge(left, right);
	...
	int yLast1 = (int) (pSample1[0] * (float) halfCanvasHeight) + halfCanvasHeight;
	int samIncrement1 = 1;
	for (int a = samIncrement1, c = 0; c < canvasWidth; a += samIncrement1, c++) {
	    int yNow = (int) (pSample1[a] * (float) halfCanvasHeight) + halfCanvasHeight;
	    gc.strokeLine(c, yLast1, c + 1.00, yNow);
	    yLast1 = yNow;
	}
	...
	if (stereo) {
	    ...
	    float[] pSample2 = right;

	    int yLast2 = (int) (pSample2[0] * (float) halfCanvasHeight) + halfCanvasHeight;
	    int samIncrement2 = 1;
	    for (int a = samIncrement2, c = 0; c < canvasWidth; a += samIncrement2, c++) {
		int yNow = (int) (pSample2[a] * (float) halfCanvasHeight) + halfCanvasHeight;
		gc.strokeLine(c, yLast2, c + 1.00, yNow);
		yLast2 = yNow;
	    }
	...
	int newSampleCount = (int) (sourceDataLine.getFormat().getFrameRate() * 0.023);
	bandWidth = (float) canvasWidth / (float) newSampleCount;
	int halfHeight = canvasHeight / 2;
	int quarterHeight = canvasHeight / 4;
	xOld = 0;
	yOld = 0;
	...
	for (int i = 0; i < newSampleCount; i++) {
	    x = (int) (i * bandWidth);
	    y = halfHeight + (int) (quarterHeight * (left[i] + right[i]));
	...
	for (int band = 0; band < saBands; band++) {
	    drawSpectrumBar((int) c, canvasHeight, (int) barWidth - 1, (int) (array[band] * canvasHeight),
		    band);
	    c += barWidth;
	}
    }
