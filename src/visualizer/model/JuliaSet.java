...
public class JuliaSet {
	...
	for (double c = bean.getReMin(), xR = 0; xR < visualizerDrawer.canvasWidth; c = c + precision, xR++) {
	    for (double ci = bean.getImMin(), yR = 0; yR < visualizerDrawer.canvasHeight; ci = ci + precision, yR++) {
		...
		if (convergenceValue != bean.getConvergenceSteps()) {
		    // Set color
		    visualizerDrawer.gc.setFill(getColorSchema(c1, c2));
		} else {
		    visualizerDrawer.gc.setFill(bean.getConvergenceColor());
		}
