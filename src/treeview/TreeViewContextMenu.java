...
public class TreeViewContextMenu extends ContextMenu {
	...
	public TreeViewContextMenu() {
		...
		/*add_Current.setOnAction(ac -> {
			if (Main.simpleMode.multiplePlaylists.getSelectedPlayList().controller.isFree(true))
				doTheInsert();
		...
		open_Current.setOnAction(ac -> {
			if (Main.simpleMode.multiplePlaylists.getSelectedPlayList().controller.isFree(true)) {
				// Προηποθέτει και μία συνολική εκάθαρση πριν την εισαγωγή των
				// στοιχείων
				Main.simpleMode.multiplePlaylists.getSelectedPlayList().controller.clearAllItems();
				doTheInsert();
			}
		});*/
	 ...
	 */
	/*public void doTheInsert() {
		// Κανονικά
		if (category == Genre.SYSTEMFILE)
			Main.simpleMode.multiplePlaylists.getSelectedPlayList().ioService.start(Arrays.asList(new File(fullPath)));
		else if (category == Genre.LIBRARY && Main.libraryMode.getLibraryWithName(fullPath).controller.isFree(true)) {
			Main.simpleMode.multiplePlaylists.getSelectedPlayList().ioService.start(fullPath);
		}
	}*/
