		...
		return new Task<Void>() {
			...
			protected Void call() throws Exception {
				...
				if (smartController.getGenre() == Genre.SEARCHWINDOW) {
					...
					smartController.setTotalInDataBase(
							observableList.stream().mapToInt(library -> ((Library) library).getTotalEntries()).sum());
					...
					Main.libraryMode.viewer.getItemsObservableList().forEach(lib -> {
						if (observableList.indexOf(lib) != observableList.size() - 1)
							queryArray
									.add(" SELECT * FROM '" + ((Library) lib).getDataBaseTableName() + "' UNION ALL ");
						else
					...
					if (Main.settingsWindow.getPlayListsSettingsController().getFileSearchGroup().getToggles().get(0)
						...
						queryArray.add(" ) WHERE replace(path, rtrim(path, replace(path,'" + File.separator
								+ "','')),'') LIKE '%" + word + "%' GROUP BY PATH LIMIT "
								+ smartController.getMaximumPerPage() + " ");
				...
				// --------------NORMAL PLAYLISTS SEARCH----------------------------
					...
					String wordQuery = smartController.getAlphabetBar().isLetterPressed() ? word + "%"
							: ("%" + word + "%");
					...
					if (Main.settingsWindow.getPlayListsSettingsController().getFileSearchGroup().getToggles().get(0)
						...
						query = query + " WHERE replace(path, rtrim(path, replace(path, '" + File.separator
								+ "', '')), '') LIKE '" + wordQuery + "' LIMIT " + smartController.getMaximumPerPage();
				}
				...
				try (ResultSet resultSet = Main.dbManager.getConnection().createStatement().executeQuery(query)) {
					...
					// Main.dbManager.connection1.createStatement().executeQuery(query)) {
					...
					while (resultSet.next()) {
						...
						arrayList.add(new Audio(resultSet.getString("PATH"), resultSet.getDouble("STARS"),
