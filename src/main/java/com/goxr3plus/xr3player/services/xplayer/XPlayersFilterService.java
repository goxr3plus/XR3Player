...
class XPlayersFilterService {
	...
	private final ExecutorService executors = Executors
			.newSingleThreadExecutor(new ThreadFactoryWithNamePrefix("XPlayers Filter Service "));

	private XPlayersFilterService() {
		// NOT USE THIS CLASS
	}
	...
	public void start() {
		Runnable runnable = () -> {
			try {
				...
				// Main.topBar.isTabSelected(Main.topBar.getMainModeTab()))
				// return true;
				// //For other players
				// else if (xPlayerController.getKey() != 0 &&
				// Main.topBar.isTabSelected(Main.topBar.getDjModeTab()))
				// return true;
				// }
				// return false;
				// }).forEach(xPlayerController -> {
				//
				// //-------Set the appropriate image for the PlayPauseButton based on the
				// status of the Player
				// ( (ImageView) xPlayerController.getPlayPauseButton().getGraphic() )
				// .setImage(xPlayerController.getxPlayer().isPlaying() ?
				// XPlayerController.pauseImage : XPlayerController.playImage);
				//
				// //-------Set the appropriate image for the PlayPauseButton based on the
				// status of the Player
