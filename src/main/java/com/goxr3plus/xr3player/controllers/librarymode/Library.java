...
public class Library extends StackPane {
	...
	/**
	 * // create a rotation transform starting at 0 degrees, rotating about pivot
	 * point 0, 0.
	 */
	...
	/**
	 * Updates the position variable of Library in database so the next time viewer
	 * position it correct.
	 *
	 * @param newPosition The new position of the Library
	 */
	// public void updatePosition(int newPosition) {
	// try (PreparedStatement libUPosition =
	// Main.dbManager.getConnection().prepareStatement("UPDATE LIBRARIES SET
	// POSITION=? WHERE NAME=?;")) {
	...
	private boolean updateImagePathInDB(String absoluteFilePath, boolean renameOperation, boolean commit) {
		...
		try (PreparedStatement libUImage = Main.dbManager.getConnection()
			...
			if (renameOperation && imageName != null) {
				...
				// logger.log(Level.WARNING, "Failed to delete image for LibraryName=[" +
				// getLibraryName() + "]");
				//
				...
				// logger.log(Level.WARNING, "Failed to create image for LibraryName=[" +
				// getLibraryName() + "]");
			}
	...
	public void setDefaultImage() {
		...
		try (PreparedStatement libUImage = Main.dbManager.getConnection()
			...
			// Ask if user is sure...
			if (ActionTool.doQuestion("Reset Image", "Reset to default the image of this library?", this,
					Main.window)) {
	...
	public void exportImage() {
		...
		new Thread(() -> {
			if (!ActionTool.copy(getAbsoluteImagePath(), file.getAbsolutePath())) {
				...
				Platform.runLater(() -> ActionTool.showNotification("Exported Image",
	...
	public void deleteLibrary(Node node, boolean byPassQuestion) {
		...
		if (!byPassQuestion)
			questionPass = ActionTool.doQuestion("Delete Library",
